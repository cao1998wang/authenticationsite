    ...
    def translate_export(message, &block)
      ...
      0.step(max, limit) do |offset|
        ...
        the_connections.each do |connection|
          ...
          parameters_json = template_parameters.to_json
          body_json = JSON.parse(translation_result)
          #rb_hash = {:parameters =>  JSON.parse(parameters_json)}.merge({:Message => body_json})
          rb_hash = {:parameters =>  JSON.parse(parameters_json)}.merge( body_json)
          rb_json = JSON.generate(rb_hash)
          ...
          if translation_result.is_a?(String)
            ...
            begin
              http_response = HTTParty.send(webhook.method, conformed_url + '/' + conformed_path, {body: rb_json, headers: headers})
